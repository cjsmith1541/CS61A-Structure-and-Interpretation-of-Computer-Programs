...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    i, j = 0, 0
    dice_number = strategy(i, j)
    while i < goal:
        while j < goal:
            if strategy(i, j) != dice_number:
                return False
            j += 1
        j = 0
        i += 1
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    dice_number, max_scoring, best_average = 1, 0, 0
    averaged_dice = make_averaged(roll_dice, total_samples)
    while dice_number <= 10:
        average = averaged_dice(dice_number, dice)
        if average > best_average:
            best_average, max_scoring = average, dice_number
        dice_number += 1
    return max_scoring
    # END PROBLEM 9
